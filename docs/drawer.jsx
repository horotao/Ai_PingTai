// Parameters drawer (advanced params)
function ParamsDrawer({ open, onClose, model, setModel, ratio, setRatio, n, setN, adv, setAdv }) {
  if (!open) return null;
  const m = MODELS[model];
  const is4k = adv.resolution === '4k';
  const availableSizes = is4k && m.sizes4k ? m.sizes4k : m.sizes;

  function patch(kv) { setAdv({ ...adv, ...kv }); }
      ...
      <div className="drawer">
        ...
        <div className="drawer-body">
          ...
          <div className="field">
            ...
            <div className="grid-opts" style={{ '--gcols': 5 }}>
              {m.sizes.map(s => {
          ...
          <div className="field">
            ...
            <div className="grid-opts" style={{ '--gcols': 4 }}>
              {[1,2,3,4].map(v => (
          ...
          {model === 'gpt-image-2-official' && (
            <React.Fragment>
              <div className="field">
                ...
                <div className="grid-opts" style={{ '--gcols': 3 }}>
                  {m.resolutions.map(r => (
              ...
              <div className="field">
                ...
                <div className="grid-opts" style={{ '--gcols': 4 }}>
                  {m.qualities.map(q => (
                    <button key={q} className={clsx('opt', q === adv.quality && 'active')} onClick={() => patch({ quality: q })}>
                      {q}
                    </button>
                  ))}
                </div>
              ...
              <div className="field">
                ...
                <input className="input-box" placeholder="https://..." style={{ marginTop: 8 }} />
                <div className="desc">需要至少一张参考图，仅对该区域重新生成。</div>
              </div>
            </React.Fragment>
          )}
...
Object.assign(window, { ParamsDrawer });
