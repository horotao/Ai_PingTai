...
function Settings({ configs, setConfigs }) {
  const [editing, setEditing] = useState(null); // id or 'new'
  const [reveal, setReveal] = useState({});
  const [testState, setTestState] = useState({});

  function startNew() {
    setEditing({ id: 'new', name: 'Apimart', key: '', defaultModel: 'gpt-image-2', enabled: true, useLocalEncrypt: false });
  }

  function save(cfg) {
    if (cfg.id === 'new') {
      const next = { ...cfg, id: 'cfg-' + Math.random().toString(36).slice(2, 8), createdAt: Date.now() };
      // if enabled, disable others
      const others = next.enabled ? configs.map(c => ({ ...c, enabled: false })) : configs;
      setConfigs([...others, next]);
    } else {
      const others = cfg.enabled ? configs.map(c => c.id === cfg.id ? cfg : { ...c, enabled: false }) : configs.map(c => c.id === cfg.id ? cfg : c);
      setConfigs(others);
    }
    setEditing(null);
  }
  ...
  function toggleEnabled(id) {
    setConfigs(configs.map(c => ({ ...c, enabled: c.id === id })));
  }

  function testConnection(id) {
    setTestState({ ...testState, [id]: 'testing' });
    setTimeout(() => {
      setTestState(prev => ({ ...prev, [id]: Math.random() > 0.1 ? 'ok' : 'fail' }));
            ...
            <div className="config-row" key={c.id}>
              ...
              <div className="info">
                <div className="name">
                  ...
                  {testState[c.id] === 'testing' && <span className="tag" style={{ background: 'transparent', borderColor: 'var(--border-2)', color: 'var(--fg-3)' }}>测试中...</span>}
                  {testState[c.id] === 'ok' && <span className="tag">连接正常</span>}
                  {testState[c.id] === 'fail' && <span className="tag" style={{ background: 'oklch(0.68 0.18 25 / 0.12)', borderColor: 'oklch(0.68 0.18 25 / 0.3)', color: 'var(--danger)' }}>连接失败</span>}
                </div>
                <div className="sub">
              ...
              <div className="ops">
                ...
                <button onClick={() => setEditing(c)} title="编辑"><I.Edit style={{ width: 15, height: 15 }} /></button>
          ...
          <div className="safety-note">
            ...
            <div>
              ...
              <ul>
                ...
                <li>可选：启用"本地加密密码"后，Key 将通过 Web Crypto API 加密保存。</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button className="btn"><I.Download style={{ width: 13, height: 13 }} /> 导出全部（JSON）</button>
              <button className="btn ghost"><I.Upload style={{ width: 13, height: 13 }} /> 从 JSON 导入</button>
              <button className="btn danger"><I.Trash style={{ width: 13, height: 13 }} /> 清空全部数据</button>
            </div>
          </div>
        </div>
      </div>
...
function EditConfigModal({ cfg, onCancel, onSave }) {
      ...
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="form-field">
            ...
            <div className="grid-opts" style={{ '--gcols': 2 }}>
              {Object.keys(MODELS).map(k => (
                <button key={k} className={clsx('opt', k === form.defaultModel && 'active')} onClick={() => setForm({ ...form, defaultModel: k })} style={{ padding: 10, textAlign: 'left' }}>
                  ...
                  <div style={{ fontSize: 10, color: 'var(--fg-4)', marginTop: 3 }}>{MODELS[k].sub}</div>
                </button>
              ))}
            </div>
          </div>
